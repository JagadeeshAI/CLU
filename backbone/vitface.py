...
class SFaceLoss(nn.Module):
    def __init__(
        ...
        self.weight = Parameter(torch.FloatTensor(out_features, in_features))
        # nn.init.xavier_uniform_(self.weight)
        nn.init.xavier_normal_(self.weight, gain=2)
                    ...
                    [
                        ...
                        Residual(
                            PreNorm(
                                dim,
                                FeedForward(
                                    dim, mlp_dim, dropout=dropout, lora_rank=lora_rank
                                ),
                            )
                        ),
                    ]
...
#             dim, depth, heads, dim_head, mlp_dim, dropout, lora_rank
#         )
...
class ViTs_face(nn.Module):
    def __init__(
        ...
        *,
        ...
        # 🔍 Step 1: Get the default args from the function signature
        frame = inspect.currentframe()
        args, _, _, values = inspect.getargvalues(frame)
        signature = inspect.signature(self.__init__)
        default_args = {k: v.default for k, v in signature.parameters.items()}

        # 🔍 Step 2: Identify non-default arguments passed
        explicitly_passed = {
